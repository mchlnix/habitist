...
class HabiticaHelper:
    ...
    def get_tag_id_by_name(self, tag_name, use_cache=True):
        """
        ...
        for tag in self.tag_cache:
            ...
            r.post("https://habitica.com/api/v3/tags", headers=self.headers, json={"name": tag_name})
    ...
    def update_task(self, habit_id, task):
        ...
        """
        resp = r.put(f"https://habitica.com/api/v3/tasks/{habit_id}", headers=self.headers, json=task)

        return resp.json()["data"]

    def download_task(self, habit_id):
        resp = r.get(f"https://habitica.com/api/v3/tasks/{habit_id}", headers=self.headers)

        return resp.json()["data"]

    def delete_task(self, habit_id):
        r.delete(f"https://habitica.com/api/v3/tasks/{habit_id}", headers=self.headers)

        return

    def upload_checklist_item(self, task, checklist_id):
        resp = r.post(f"https://habitica.com/api/v3/tasks/{checklist_id}/checklist", headers=self.headers, json=task)
    ...
    def score_task(self, habit_id, direction="up"):
        r.post(f"https://habitica.com/api/v3/tasks/{habit_id}/score/{direction}", headers=self.headers)
    ...
    def score_checklist_item(self, item_id, parent_id):
        r.post(f"https://habitica.com/api/v3/tasks/{parent_id}/checklist/{item_id}/score", headers=self.headers)

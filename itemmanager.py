        ...
        for _item in self.item_list.values():
            ...
            try:
                if not self.habit.checklist_item_is_uploaded(_item.habit_id, self.item_list[_item.parent].habit_id):
                    _item.habit_id = self.habit.upload_checklist_item({"text": _item.content},
                                                                      self.item_list[_item.parent].habit_id)
            except Exception as e:
                print(str(e))
                print("Failed uploading checklist item: %s" % _item.content)

            if _item.completed != self.habit.checklist_item_is_completed(
                    _item.habit_id, self.item_list[_item.parent].habit_id
            ):  # no support for repeating subtasks
